...
var dumpSVG = function() {
  var cv = document.getElementById("cv");
  var cvx = cv.getContext("2d");
  canvas_svg = new C2S(cv.width,cv.height); //width, height of your desired svg file
  canvas_svg.drawImage(cv, 0,0);
  svg = canvas_svg.getSerializedSvg(true);
  var b64 = window.btoa(unescape(encodeURIComponent(svg))); // or use btoa if supported
  document.getElementById('svg').innerHTML = "<a href-lang='image/svg+xml' href='data:image/svg+xml;base64,\n"+b64+"' title='file.svg'>Download</a>";   
}
...
var dumpVars = function() {
  for (var i = 1 ; i < qArray.length ; i++) {
    //Origineel: console.log(t, qArray[i].qType, qArray[i].qName, qArray[i].j, qArray[i].k)
	
		if(qArray[i].qName == 'fractionOfInputsAllocatedToLandMaintenance' || qArray[i].qName == 'food' || qArray[i].qName == 'agriculturalInputs' || qArray[i].qName == 'population' || qArray[i].qName == 'serviceOutput' || qArray[i].qName == 'fractionOfIndustrialOutputAllocatedToAgriculture' || qArray[i].qName == 'industrialOutput' || qArray[i].qName == 'indexOfPersistentPollution'){
